...
classes:{},build:"bs4/dt-1.10.18/rg-1.1.0/rr-1.2.4/sl-1.3.0",errMode:"alert",feature:[],search:[],selector:{cell:[],column:[],row:[]},internal:{},legacy:{ajax:null},pager:{},renderer:{pageButton:{},header:{}},order:{},type:{detect:[],search:{},order:{}},_unique:0,fnVersionCheck:n.fnVersionCheck,iApiIndex:0,oJUIClasses:{},sVersion:n.version};h.extend(x,{afnFiltering:x.search,aTypes:x.type.detect,ofnSearch:x.type.search,oSort:x.type.order,afnSortData:x.order,aoFeatures:x.feature,oApi:x.internal,oStdClasses:x.classes,oPagination:x.pager});
...
e=this.dom.dtScroll;this.s.scroll={windowHeight:d(f).height(),windowWidth:d(f).width(),dtTop:e.length?e.offset().top:null,dtLeft:e.length?e.offset().left:null,dtHeight:e.length?e.outerHeight():null,dtWidth:e.length?e.outerWidth():null}},_mouseMove:function(c){this._clonePosition( c );var bodyY = this._eventToPage( c, 'Y' ) - this.s.bodyTop;var middles = this.s.middles;var insertPoint = null;var dt = this.s.dt;var body = dt.table().body();for ( var i=0, ien=middles.length ; i<ien ; i++ ) {if ( bodyY < middles[i] ) {insertPoint = i;break;}}if ( insertPoint === null ) {insertPoint = middles.length;}if ( this.s.lastInsert === null || this.s.lastInsert !== insertPoint ) {var crows = dt.rows( { page: 'current' } );var unodes = $.unique( crows.nodes().toArray() );var data = crows.data();var rrows = this.dom.target.closest('tbody').find('tr[role="row"]');var backRowPoint = rrows.index(this.dom.target);if (insertPoint === backRowPoint || insertPoint === backRowPoint + 1) {this.s.lastInsert = insertPoint;return;}var downside = this.s.lastInsert === undefined || insertPoint > this.s.lastInsert;var exchPoint = downside ? insertPoint - 1 : insertPoint;if (dt.settings()[0].rowGroup !== undefined) {var colName = dt.settings()[0].rowGroup.c.dataSrc;var oldGroup = dt.row(this.dom.target).data()[colName];var newGroup = oldGroup;newGroup = data[exchPoint][colName];if (newGroup !== oldGroup) return;}var exchNoCell = crows.nodes().to$().find('td:first-child .reorderno').eq(exchPoint);var backNoCell = this.dom.target.find('td .reorderno').eq(0);var overNoCell = this.dom.clone.find('td .reorderno').eq(0);var newVal = exchNoCell.html();var oldVal = backNoCell.html();exchNoCell.html(oldVal);backNoCell.html(newVal);overNoCell.html(newVal);if ( insertPoint > this.s.lastInsert ) {this.dom.target.insertAfter(unodes[exchPoint]);}else {this.dom.target.insertBefore(unodes[exchPoint]);}this._cachePositions();this.s.lastInsert = insertPoint;}this._shiftScroll( c );},_mouseUp:function(){var c=this,b=this.s.dt,a,e,i=this.c.dataSrc;this.dom.clone.remove();this.dom.clone=null;this.dom.target.removeClass("dt-rowReorder-moving");d(g).off(".rowReorder");d(g.body).removeClass("dt-rowReorder-noOverflow");clearInterval(this.s.scrollInterval);
